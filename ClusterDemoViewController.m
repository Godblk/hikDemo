...
static const CGFloat kZoomLevel = 21;
...
- (void)viewDidLoad {
    ...
    for (NSInteger i = 3; i < 23; i++) {
        [_clusterCaches addObject:[NSMutableArray array]];
    }
    UIBarButtonItem *item = [[UIBarButtonItem alloc] initWithTitle:@"点我" style:UIBarButtonItemStyleDone target:self action:@selector(onClick)];
    self.navigationItem.rightBarButtonItem = item;
    //点聚合管理类
    _clusterManager = [[BMKClusterManager alloc] init];
    CLLocationCoordinate2D coor = CLLocationCoordinate2DMake(39.915, 116.404);
    //向点聚合管理类中添加标注
    NSMutableArray *clusters = [_clusterCaches objectAtIndex:(kZoomLevel - 3)];
    ...
    for (NSInteger i = 0; i < 20; i++) {
        ...
        annotation.coordinate = clusterItem.coor;
        annotation.size = 1;
        annotation.title = [NSString stringWithFormat:@"我是%d个", 1];
...
- (void)onClick {
    [_mapView setZoomLevel:kZoomLevel];
}
...
//            [_mapView addAnnotations:clusters];
            for (ClusterAnnotation *anno in clusters) {
                BOOL isContain = BMKMapRectContainsPoint(self.mapView.visibleMapRect,BMKMapPointForCoordinate(anno.coordinate));
                if (isContain) {
            ...
            dispatch_async(dispatch_get_global_queue(0, 0), ^{
                ...
                dispatch_async(dispatch_get_main_queue(), ^{
                    for (BMKCluster *item in array) {
                        ...
                        annotation.childItems = item.directItems;
                        annotation.title = [NSString stringWithFormat:@"我是%ld个", item.size];
                        [clusters addObject:annotation];
                        
                        BOOL isContain = BMKMapRectContainsPoint(self.mapView.visibleMapRect,BMKMapPointForCoordinate(item.coordinate));
                        NSLog(@"%@",isContain?@"包含":@"不包含");
                    }
                    [_mapView removeAnnotations:_mapView.annotations];
                    [_mapView addAnnotations:clusters];
...
//    ClusterAnnotation *annotation = view.annotation;
//    if (annotation.size > 1) {
//        NSMutableArray *annotations = [NSMutableArray array];
//        for (BMKClusterItem *item in annotation.childItems) {
//            ClusterAnnotation *annotation = [[ClusterAnnotation alloc] init];
//            annotation.coordinate = item.coor;
//            annotation.size = 1;
//            annotation.title = [NSString stringWithFormat:@"我是%d个", 1];
